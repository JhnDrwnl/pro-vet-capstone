...
router.post('/send-appointment-reschedule', async (req, res) => {
  try {
    const { phoneNumber, message, type } = req.body;
    ...
    // Use the existing sendTestMessage function for appointment reschedules
    const result = await smsService.sendTestMessage(phoneNumber, message);
    ...
    res.json({ 
      ...
      messageId: result.messageId,
      status: result.status,
      message: 'Appointment reschedule SMS sent successfully'
    });
    
  } catch (error) {
    console.error('SMS send-appointment-reschedule error:', error);
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Failed to send appointment reschedule SMS',
      error: error.message 
    });
  }
});
...
router.post('/send-appointment-completion', async (req, res) => {
  try {
    const { phoneNumber, message, type } = req.body;
    ...
    // Use the existing sendTestMessage function for appointment completions
    const result = await smsService.sendTestMessage(phoneNumber, message);
    ...
    res.json({ 
      ...
      messageId: result.messageId,
      status: result.status,
      message: 'Appointment completion SMS sent successfully'
    });
    
  } catch (error) {
    console.error('SMS send-appointment-completion error:', error);
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Failed to send appointment completion SMS',
      error: error.message 
    });
  }
});
