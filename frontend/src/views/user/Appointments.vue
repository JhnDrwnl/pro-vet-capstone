...
<template>
            ...
            <div class="bg-white rounded-lg shadow-lg p-3 md:p-4">
              ...
              <div class="grid grid-cols-7 gap-1 md:gap-2">
                ...
                <button
                  ...
                  @click="isWeekend || !isDayAvailable(date) ? showWeekendModal() : selectDate(date)"
                  :disabled="!isCurrentMonth || isWeekend || !isDayAvailable(date)"
                  class="p-1 md:p-2 rounded-full text-xs md:text-sm relative"
                  :class="[
                    isCurrentMonth ? (
                      isWeekend || !isDayAvailable(date) ? 'text-red-400 hover:bg-red-50 cursor-not-allowed' : 'hover:bg-blue-800'
                    ) : 'text-gray-300',
                    ...
                    !isDayAvailable(date) && isCurrentMonth && !isWeekend ? 'bg-gray-200 text-gray-400' : '',
                    // Add class for available days
                    isDayAvailable(date) ? 'border border-green-400' : ''
                  ]"
                >
                  {{ date.getDate() }}
                </button>
  ...
  import { startOfMonth, endOfMonth, eachDayOfInterval, format, addMonths, subMonths, parse, addMinutes, setHours, setMinutes, startOfDay, endOfDay, isToday, isSameDay } from 'date-fns'
  ...
  const isDayAvailable = (date) => {
    ...
    if (officeStore.checkHolidayStatus(date.toISOString().split('T')[0])) {
      return false
    }
    ...
    // Check if the day has schedules in Firestore - safely check if function exists
    const dateString = date.toISOString().split('T')[0]
    
    // Check if hasScheduleForDate function exists in officeStore
    if (typeof officeStore.hasScheduleForDate === 'function') {
      return officeStore.hasScheduleForDate(dateString)
    }
    
    // If the function doesn't exist, default to true for weekdays with open office hours
    return true
  }
  ...
  const filteredUserPets = computed(() => {
    ...
    return userPets.value.filter(pet => 
      pet.species && pet.species.toLowerCase() === selectedSpecies.value.toLowerCase()
    )
  })
  ...
  // Function to get default office hours
  const getDefaultOfficeHours = () => {
    // Try to get office hours from the store
    if (officeStore && officeStore.getOfficeHoursForDay) {
      // Get office hours for Monday as default
      const officeHoursData = officeStore.getOfficeHoursForDay('Monday')
      if (officeHoursData && officeHoursData.isOpen) {
        return {
          openHour: parseInt(officeHoursData.openTime.split(':')[0]),
          openMinute: parseInt(officeHoursData.openTime.split(':')[1]),
          closeHour: parseInt(officeHoursData.closeTime.split(':')[0]),
          closeMinute: parseInt(officeHoursData.closeTime.split(':')[1]),
          hasLunchBreak: !!(officeHoursData.lunchStart && officeHoursData.lunchEnd),
          lunchStartHour: officeHoursData.lunchStart ? parseInt(officeHoursData.lunchStart.split(':')[0]) : null,
          lunchStartMinute: officeHoursData.lunchStart ? parseInt(officeHoursData.lunchStart.split(':')[1]) : null,
          lunchEndHour: officeHoursData.lunchEnd ? parseInt(officeHoursData.lunchEnd.split(':')[0]) : null,
          lunchEndMinute: officeHoursData.lunchStart ? parseInt(officeHoursData.lunchStart.split(':')[1]) : null
        }
      }
    }
    
    // Default office hours if not available from store
    return {
      openHour: 8,
      openMinute: 0,
      closeHour: 17,
  ...
  // Update the available time slots computed property to use the timeRange for display
  ...
  watch(() => selectedDoctor.value, (newDoctor, oldDoctor) => {
    if (newDoctor) {
  ...
  watch(() => currentStep.value.id, (newStepId, oldStepId) => {
    ...
    if (newStepId === 'datetime' && !selectedDate.value) {
      ...
      if (!isWeekend(today) && isDayAvailable(today)) {
        selectDate(today)
      }
    }
  })
  ...
  // Update the bookAppointment function to use the time range
  const bookAppointment = async () => {
    ...
    try {
      ...
      const serviceNames = selectedServices.value.map(id => getServiceNameWithDuration(id));
      ...
      const appointmentData = {
        ...
        doctorName: `Dr. ${selectedDoctor.value.firstName} ${selectedDoctor.lastName}`,
  ...
  const setDefaultDateToToday = () => {
    if (!selectedDate.value) {
      ...
      // Only set today as default if it's not a weekend and is available
      if (!isWeekend(today) && isDayAvailable(today)) {
        ...
        while (daysToCheck > 0) {
          ...
          if (!isWeekend(nextAvailableDay) && isDayAvailable(nextAvailableDay)) {
  ...
  onMounted(async () => {
    ...
    try {
      ...
      // Fetch office hours from the store
      await fetchOfficeHours()
      
      // Fetch categories and services from the store
  ...
  // Define a ref to hold the base class for the step circles
  const baseStepCircleClass = 'w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center transition-all duration-200 step-circle';
