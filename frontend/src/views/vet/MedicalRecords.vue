<template>
  <div class="telehealth-container">
    ...
    <div v-else-if="currentView === 'sessions' && !activeSession" class="sessions-list">
      <div class="flex items-center mb-4">
        ...
        <h1 class="text-2xl font-bold">My Telehealth Sessions</h1>
      </div>
  ...
  const refreshSessions = async () => {
    ...
    try {
      ...
      const vetId = auth.currentUser?.uid || 'test-doctor-id'; // Fallback for testing
      
      const q = query(
        collection(db, 'telehealth_sessions'),
        where(userType.value === 'vet' ? 'doctorId' : 'patientId', '==', 
              userType.value === 'vet' ? vetId : 'test-patient-id')
      );
  ...
  const completeJoinSession = async (session) => {
    try {
      ...
      if (session.status === 'scheduled') {
        ...
        await updateDoc(doc(db, 'telehealth_sessions', session.id), updateData);
      }
  ...
  const initializeWebRTC = async (sessionId, existingStream = null) => {
    try {
      ...
      webRTCService = new WebRTCService({
        ...
        sessionId,
        userId: auth.currentUser?.uid || (userType.value === 'vet' ? 'test-doctor-id' : 'test-patient-id'),
