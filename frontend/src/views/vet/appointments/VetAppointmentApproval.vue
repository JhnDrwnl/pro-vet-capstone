  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
    <div class="flex items-center gap-2 w-full sm:w-auto">
      ...
      <div class="relative">
        ...
        <!-- Filter Dropdown - Status Only (removed processing) -->
          ...
          <div v-if="currentApprovalStep.id === 'confirm'" class="space-y-4">
            <div class="bg-white rounded-lg p-4 border border-gray-200">
              ...
              <div class="space-y-4">
                ...
                <!-- Pet Summary - Enhanced with all details -->
                <div class="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div class="flex items-start">
                    ...
                    <div class="flex-1">
                      ...
                      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-2 mt-2">
                        ...
                        <div>
                          <div class="text-xs text-gray-500">Gender</div>
                          <div class="text-sm text-gray-700 capitalize">{{ formatGender(selectedAppointment.petGender) || 'Not provided' }}</div>
                        </div>
                        <div>
                          <div class="text-xs text-gray-500">Age</div>
...
const startApprovalProcess = async (appointment) => {
  ...
  try {
    ...
    // Fetch complete pet data if needed
    let petData = {};
    if (appointment.petId) {
      const petPromise = petsStore.getPetById(appointment.userId, appointment.petId)
        .then(pet => {
...
const cancelProcessing = async (appointment) => {
  ...
  try {
    ...
    // Update the appointment status back to the original status
    const updatedAppointment = await appointmentStore.updateAppointmentStatus(appointment.id, originalStatus);
    ...
    if (updatedAppointment) {
      ...
      if (index !== -1) {
        appointments.value[index] = {
          ...updatedAppointment,
          // Preserve any local-only properties
          ownerName: appointments.value[index].ownerName,
          ownerAvatar: appointments.value[index].ownerAvatar,
          ownerEmail: appointments.value[index].ownerEmail,
          contactInformation: appointments.value[index].contactInformation,
...
watch(() => appointmentStore.appointments, (newAppointments) => {
  ...
  if (newAppointments.length > 0) {
    ...
    appointments.value = filteredAppointments.map(storeAppointment => {
      ...
      if (localAppointment) {
        // Don't override processing status if we're actively working on this appointment
        if (localAppointment.pendingAction) {
          return {
            ...localAppointment,
            // Only update non-status fields from the store
            createdAt: storeAppointment.createdAt,
            updatedAt: storeAppointment.updatedAt,
            // Keep the status and pendingAction from local state
            status: localAppointment.status,
            pendingAction: localAppointment.pendingAction
          };
        }
        ...
        return {
          ...
          ownerName: localAppointment.ownerName,
          ownerAvatar: localAppointment.ownerAvatar,
          ownerEmail: localAppointment.ownerEmail,
...
const closeCancelModal = async () => {
  ...
  if (selectedAppointment.value) {
    ...
    try {
      ...
      // Update the appointment status back to the original status in Firestore
