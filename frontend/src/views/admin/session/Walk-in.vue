<template>
  <div class="bg-white p-6 rounded-2xl">
    ...
    <div class="mb-6">
      ...
      <p class="text-gray-500 mt-1">Manage walk-in consultation sessions.</p>
    </div>
      ...
      <table class="min-w-full">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="session in paginatedSessions" :key="session.id" 
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span :class="getUrgencyClass(session.urgencyLevel)">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              v-model="sessionForm.urgencyLevel"
              required
              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-200"
            >
...
const sessions = ref([
  ...
  },
  {
    ...
    reasonForVisit: 'Severe vomiting',
...
const exportToCSV = () => {
  const headers = ['Client Name', 'Contact', 'Pet Details', 'Date & Time', 'Reason', 'Veterinarian', 'Urgency'];
  const csvContent = [
    headers.join(','),
  ...
  link.download = 'walk-in-sessions.csv';
  link.click();
};
</script>
