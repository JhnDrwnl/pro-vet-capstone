...
<template>
  ...
  const emit = defineEmits(['update:modelValue', 'dateSelected', 'monthSelected', 'yearSelected', 'cancel', 'clear']);
  ...
  const navigate = (direction) => {
    if (view.value === 'days') {
      ...
      emit('monthSelected', { month: currentMonth.value, year: currentYear.value });
    } else if (view.value === 'months') {
      currentYear.value += direction === 'prev' ? -1 : 1;
      emit('yearSelected', currentYear.value);
  ...
  const selectYear = (year) => {
    ...
    view.value = 'months';
    const selectedDate = new Date(year, currentMonth.value, 1);
    const formattedDate = formatDate(selectedDate);
    emit('update:modelValue', formattedDate);
    emit('dateSelected', formattedDate);
  };
