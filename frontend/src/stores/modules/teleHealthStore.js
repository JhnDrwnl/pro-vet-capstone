...
import { ref as dbRef, set, push, update, onValue, onDisconnect } from "firebase/database"
...
export const useTelehealthStore = defineStore("telehealth", () => {
  ...
  const loadUserSessions = async () => {
    ...
    try {
      const role = isDoctor.value ? "doctor" : "patient"
      const sessions = await TelehealthService.getUserSessions(currentUser.value.uid, role)

      // Split into upcoming and past sessions
      const now = new Date()
      upcomingSessions.value = sessions
        .filter((session) => new Date(session.scheduledTime) > now && session.status !== "cancelled")
