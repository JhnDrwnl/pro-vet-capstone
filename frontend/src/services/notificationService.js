...
class NotificationService {
  ...
  cleanupProcessedNotifications() {
    if (this.processedNotifications.size > 100) {
      ...
      console.log(
        `Cleaned up processed notifications cache, now tracking ${this.processedNotifications.size} notifications`,
      )
    }
  }
  ...
  async isNotificationAlreadyInFirestore(userId, notificationId) {
    try {
      ...
      const q = query(notificationsRef, where("userId", "==", userId), where("notificationId", "==", notificationId))
